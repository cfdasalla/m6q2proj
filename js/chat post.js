			...
			messagesToAdd.unshift(function() {
				setTimeout(function() {
					addMessage(x[1], x[2], function() {
						...
						}, x[2] == "left" ? 0 : i == 0 ? 0 : timeDelay($("#chat div[class~=left], #chat div[class~=right]").last().get(0).innerText.length, true));
					}, timeDelay(x[1].length));
				}, i == 0 ? 0 : timeDelay($("#chat div[class~=left], #chat div[class~=right]").last().get(0).innerText.length, true));
			});
		} else {
			messagesToAdd.unshift(function() {
				setTimeout(function() {
