...
let messages = [
	["m", "Uy, Cal! 👋🏽", left],
	["m", "Sagutan na natin yung pair work!", left],
	["m", "Sige!", right],
	["m", "Yun nga lang...", right],
	["m", "...hindi ko pa masyado gets yung concepts 😣", right],
	["m", "Ganun ba?", left],
	["m", "Turuan na lang kita habang ginagawa natin.", left],
	["m", "So yung topic kasi natin ay reverse power rule.", left],
	["m", "Naalala mo pa ba yung power rule na ginawa natin dati sa derivatives?", left],
	["m", "Medyo...", right],
	["m", "Masasabi nating medyo baliktad yung strategy na ’to sa reverse power rule na lesson natin ngayon.", left],
	["m", "Para mag-integrate gamit ang reverse power rule, kailangan nating dagdagan ng 1 yung exponent natin, tapos magdi-divide tayo ng kaparehas na number sa mismong term.", left],
	["m", "Bale, pag nag-reverse power rule tayo sa isang term, kunyari \\(ax^b\\), ang integral niya ay \\(\\frac{a}{b+1}x^{b+1}\\).", left],
	...
	["m", "Bakit?", right],
	["m", "Hindi pa dun nagtatapos ang reverse power rule.", left],
	["m", "Ha? Eh mukha namang tama yung sagot ah?", right],
	["m", "Tama naman yung sagot natin, pero may nakakalimutan tayo.", left],
	["m", "Ano ang derivative ng \\(" + r2.latex() + "\\)?", left],
	["m", "\\(" + r2.derivative().latex() + "\\), diba?", right],
	["m", "Tama...", left],
	["m", "Pero ano naman ang derivative ng \\(" + r2_a.latex() + "\\)?", left],
	["m", "\\(" + r2_a.derivative().latex() + "\\) din...", right],
	["m", "Ano naman ang derivative ng \\(" + r2_b.latex() + "\\)?", left],
	["m", "\\(" + r2_b.derivative().latex() + "\\) pa rin!", right],
	["m", "Dahil ang integration ang kabaligtaran ng differentiation, alin dun sa tatlo ang pwedeng integral ng \\(" + r2_b.derivative().latex() + "\\)?", left],
	["m", "Hala! Pwede kahit alin sa kanila! 😲", right],
	["m", "Oo. Kahit nga ano pa yung constant sa dulo, pwede pa rin siyang integral ng \\(" + r2_b.derivative().latex() + "\\).", left],
	["m", "Pa’no yun? Ano yung integral niya kung maraming pwedeng sagot? 🤔", right],
	["m", "Kelangan nating isama lahat ng pwedeng maging integral ng function na yun.", left],
	["m", "Dahil dun, kailangan nating magdagdag ng \\(+C\\) sa dulo ng makukuha natin sa reverse power rule.", left],
	["m", "Pwede nating palitan si \\(C\\) ng kahit anong real number, at tama pa rin yung sagot natin.", left],
	["m", "Ah! Ganun pala yun! Kaya pala may nakikita akong \\(C\\) dun sa libro natin.", right],
	["m", "Ngayon, balikan natin yung unang halimbawa natin.", left],
	...
	["m", "Wag mong kakalimutan yung \\(+C\\) sa dulo ng sagot pag nag-integrate ka.", left],
	["m", "Pag nalimutan mong ilagay yun, mamamali yung sagot mo.", left],
	["m", "OK! Aalalahanin ko lagi!", right],
	["m", "Nga pala...", right],
	["m", "Pwede rin bang mag-integrate nang pa-isa-isa kapag polynomial?", right],
	["m", "Ay! Buti pinaalala mo!", left],
	["m", "Gaya ng sa pag-differentiate, pwede mo ring isa-isahin ang pag-integrate sa polynomial.", left],
	["m", "Halimbawa, \\(" + r3.latex() + "\\).", left],
	...
	["m", "O, mukhang gets mo na ’tong reverse power rule.", left],
	["m", "Gawin na natin ’tong pairwork!", left],
	["m", "Number 1:", left],
	...
	["m", "O ayan! Natapos na natin yung pairwork!", left],
	["m", "Yehey! Salamat ulit sa pagturo sa ’kin! 😁", right],
	["m", "Walang anuman!", left],
	["m", "May quiz pala bukas, ha!", left],
	["m", "Good luck! Kaya mo yan!", left],
